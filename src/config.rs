...
impl Config {
    ...
    pub fn get_from_addresses(&self) -> HashMap<SocketAddr, NodeId> {
        self.nodes.iter().map(|(node_id, node_address)| (node_address.source, *node_id)).collect()
    }
}
...
fn parse_args() -> Result<(NodeId, String)> {
    ...
    if args.len() != 3 {
        return Err(anyhow!("You must provide the NodeId and path to config file as command line arguments"));
    }
...
fn parse_network_config(config_path: &str) -> Result<HashMap<NodeId, NodeAddress>> {
    ...
    config.into_iter().map(|(node_id, node_address)| -> Result<(NodeId, NodeAddress)> {
